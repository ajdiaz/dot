...
def my_print_cb(data, buffer, date, tags, displayed, highlight, prefix, message):
    ...
    if buffer in w.config_get_plugin('ignore_chans').split(','):
        return w.WEECHAT_RC_OK
    if int(highlight):
        infos = {'buffer'   : buffer,
                 'date'     : date,
...
def clear_file_cb(data, buffer, args):
    ...
    highlights = []
    write_file()
    if w.config_get_plugin('remove_on_clear') == 'on':
        shutdown()
    return w.WEECHAT_RC_OK
...
boolean_options = ['summarize_by_buffer', 'short_buffer_name',
                   'clear_visited_buffers', 'remove_on_clear']
